use sysinfo::{Signal, System, ProcessExt, SystemExt};
...
use winapi::um::winbase::{CREATE_SUSPENDED, DETACHED_PROCESS, CREATE_NO_WINDOW};
        ...
        for &target_name in process_names {
            if process.name().eq_ignore_ascii_case(target_name) {
                ...
                if result.is_none() || !result.unwrap() {
                    println!("Failed to kill process {}: {}", target_name, pid);
                    continue;
                }
                break;
            }
...
fn start_internal(
    ...
    let success = unsafe {
        CreateProcessW(
            ...
            null_mut() as LPSECURITY_ATTRIBUTES,
            0,
            if suspended { CREATE_SUSPENDED } else { 0 } | CREATE_NO_WINDOW,
            null_mut(),
...
pub fn start_with_args(process_path: PathBuf, args: Vec<String>) -> Result<u32, String> {
    ...
    start_internal(process_path, false, Some(args_str))
}
